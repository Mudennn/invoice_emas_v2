            ...
            <div class="invoice-details">
                @if ($pageNum === 0)
                    <div class="header">
                        <div class="company-header">
                            ...
                            <p class="company-address">{{ $ourCompany->address_line_1 }},
                                {{ $ourCompany->address_line_2 }},
                                {{ $ourCompany->postcode }}
                                {{ $ourCompany->city }},
                    ...
                    <div class="customer-details">
                        <div class="left-side">
                            <h6>Issued To:</h6>
                            <p class="customer-name">{{ $invoice->company_name }}</p>
                            @php
                        ...
                        <div class="right-side">
                            <h6 class="invoice-no">Invoice No: <span
                                    class="invoice-no-text">{{ str_replace('INV', 'INV ', $invoice->invoice_no) }}</span></h6>
                ...
                @else
                    <div class="continuation-header">
                        ...
                        <div class="right-side">
                            <p class="invoice-no">Invoice No: <span
                                    class="invoice-no-text">{{ str_replace('INV', 'INV ', $invoice->invoice_no) }}</span></p>
                            <p class="invoice-date">Date:
                                {{ \Carbon\Carbon::parse($invoice->invoice_date)->format('d F Y') }}</p>
                            <p class="page-number">Page {{ $pageNum + 1 }} of {{ $totalPages }}</p>
                        </div>
                    ...
                    <table class="invoice-table">
                        ...
                        <tbody>
                            @foreach ($pageItems as $index => $item)
                                <tr>
                                    ...
                                    <td class="total-data">
                                        {{ number_format($item->quantity) == 0 ? '' : number_format($item->quantity) }}
                                        {{ $item->s_pair }} </td>
                                    <td class="total-data">
                                        {{ number_format($item->weight, 2) == 0 ? '' : number_format($item->weight, 2) }}
                                    </td>
                                    <td class="total-data">
                                        {{ number_format($item->wastage) == 0 ? '' : number_format($item->wastage) . '%' }}
                                    </td>
                                    <td class="total-data">
                                        {{ number_format($item->total_weight, 2) == 0 ? '' : number_format($item->total_weight, 2) }}
                                    </td>
                                    <td>
                                        @if ($item->gold == 0)
                                    ...
                                    <td class="total-data">{{ $item->remark_total == 0 ? '' : $item->remark_total }}</td>
                                    </td>
                                    <td>
                        ...
                        @if ($pageNum === $totalPages - 1)
                            <tfoot class="totals-table">
                                <tr>
                                    <td colspan="10" class="total">Subtotal:</td>
                                    <td style="font-weight: bold !important;" class="total-data total">
                                        <div style="display: flex; justify-content: space-between;">
                                            <span>RM</span>
