<input type="hidden" name="id" value="{{ $debit_note->id }}">
...
<div class="credit-note-form-container">
    <div class="left-form">
        <div class="input-form">
            ...
            <input type="text" name="debit_note_no" value="{{ old('debit_note_no') ?? $debit_note->debit_note_no }}"
            ...
            <select name="invoice_no" id="invoice_no" class="form-control form-select invoice-select2"
                ...
                @foreach ($invoices ?? [] as $invoice)
                    <option value="{{ $invoice->invoice_no }}"
                        {{ (old('invoice_no') ?? $debit_note->invoice_no) == $invoice->invoice_no ? 'selected' : '' }}>
                        {{ $invoice->invoice_no }}
                    </option>
                @endforeach
            </select>
                ...
                @foreach ($reasons as $reason)
                    @if ($debit_note->reason)
                        <option value="{{ $reason->id }}"
                            {{ $reason->id == $debit_note->reason ? 'selected' : '' }}>
        ...
        <div class="input-form">
            ...
            <input type="date" name="date"
                value="{{ old('date', isset($debit_note->date) ? \Carbon\Carbon::parse($debit_note->date)->format('Y-m-d') : now()->timezone('Asia/Kuala_Lumpur')->format('Y-m-d')) }}"
                class="form-control" {{ $ro }}>
...
<div class="mt-4">
    ...
    <div class="card-body">
        <div class="table-responsive" style="overflow-y: auto;" id="debit_notes-items">
            <table class="table table-hover table-bordered align-middle text-nowrap">
                ...
                <tbody>
                    @if (isset($debit_note->debitItems) && count($debit_note->debitItems) > 0)
                        @foreach ($debit_note->debitItems->where('status', '0') as $index => $item)
                            <tr class="debit-note-item" data-item-id="{{ $item->id }}"
                    ...
                    <tr>
                        ...
                        <th class="text-end">
                            <input type="number" step="0.01" name="subtotal"
                                ...
                                value="{{ old('subtotal') ?? ($debit_note->debitItems->first()->subtotal ?? '0.00') }}"
                                readonly>
                        </th>
                        <th><input type="number" step="0.01" name="total_return_balance"
                            class="form-control text-end total-return-balance-input"
                            value="{{ old('remark_total') ?? ($debit_note->debitItems->first()->remark_total ?? '0.00') }}" 
                            readonly></th>
                        @if (!isset($ro) || $ro === '')
                            <th></th>
                        @endif
...
<div class="input-form">
    ...
    <textarea name="note" class="form-control" {{ $ro }}>{{ old('note') ?? $debit_note->note }}</textarea>
