@extends('layouts.main', ['title' => 'Debit Note Items for Debit Note #' . $debit_note->debit_note_no])

@section('content')
    <div class="table-header">
        <h1 class="h3 mb-0">Debit Note Items</h1>
        <a href="{{ route('debit_notes.index') }}" class="third-button">
    ...
    <div class="card">
        <div class="table-responsive" style="padding: 24px !important; min-height: 200px; overflow-y: auto;">
            <table class="table table-hover table-bordered align-middle text-nowrap" style=" max-width: 100%; table-layout: fixed;">
                ...
                <tbody>
                    @forelse($debit_note->debitItems as $item)
                        <tr>
                            ...
                            <td class="text-end">{{ number_format($item->weight, 2) }} </td>
                            <td class="text-end">{{ number_format($item->wastage) }}%</td>
                ...
                <tfoot>
                    <tr>
                        ...
                        <th>PG: {{ number_format($debit_note->debitItems->sum('remark_total'), 2) }}</th>
                    </tr>
                </tfoot>
            </table>
        </div>
