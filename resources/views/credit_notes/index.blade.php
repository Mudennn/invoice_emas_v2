@extends('layouts.main', ['title' => 'Credit Notes'])

@section('content')
    <div class="table-header">
        <h1 class="h3 mb-0">Credit Notes</h1>
        <a href="{{ route('credit_notes.create') }}" class="primary-button">Create Credit Note</a>
    </div>
                ...
                <tbody>
                    @foreach ($credit_notes as $index => $credit_note)
                        <tr>
                            ...
                            <td>{{ \Carbon\Carbon::parse($credit_note->date)->format('d F Y') }}</td>
                            <td class="text-wrap"> {{ $credit_note->invoice->company_name ?? 'N/A' }}</td>
                            <td>
                            ...
                            <td class="text-wrap">{{ $credit_note->note }}</td>
                            <td class="text-center">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                        <li><a href="{{ route('credit_notes.edit', $credit_note->id) }}"
                                        ...
                                        <li><a href="{{ route('credit_notes.view', $credit_note->id) }}"
                                                class="dropdown-item"
                                                style="display: flex; justify-content: start; align-items: center; gap:8px; font-size:14px;"><span
                                                    class="material-symbols-outlined" style="font-size: 14px">
                                                    list_alt
