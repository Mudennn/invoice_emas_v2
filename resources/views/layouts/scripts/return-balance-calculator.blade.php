...
const ReturnBalanceCalculator = {
    ...
    init: function() {
        // Add event listener for pure gold dropdown to handle calculations
        document.addEventListener('change', function(e) {
            if (e.target.classList.contains('pure-gold-input')) {
                ReturnBalanceCalculator.calculateReturnBalance(e.target);
            }
        });
        
        // Initial calculation for existing pure gold values
        document.querySelectorAll('.pure-gold-input').forEach(pureGoldInput => {
            if (pureGoldInput.value) {
                // Wait for other initial calculations to complete
                setTimeout(() => {
                    ReturnBalanceCalculator.calculateReturnBalance(pureGoldInput);
                }, 500);
            }
        });
        ...
        document.addEventListener('input', function(e) {
            if (e.target.classList.contains('weight-input') || e.target.classList.contains('wastage-input')) {
                ...
                setTimeout(() => {
                    ...
                    const pureGoldInput = row.querySelector('.pure-gold-input');
                    if (pureGoldInput) {
                        ReturnBalanceCalculator.calculateReturnBalance(pureGoldInput);
                    }
                }, 100);
    ...
    setupInvoiceSelectionHandler: function() {
        ...
        $('.invoice-select2').on('change', function() {
            ...
            setTimeout(() => {
                // Recalculate return balances for all pure gold inputs
                document.querySelectorAll('.pure-gold-input').forEach(pureGoldInput => {
                    if (pureGoldInput.value) {
                        ReturnBalanceCalculator.calculateReturnBalance(pureGoldInput);
                    }
    ...
    // Calculate return balance based on pure gold value
    calculateReturnBalance: function(pureGoldInput) {
        const row = pureGoldInput.closest('tr');
        const pureGoldValue = pureGoldInput.value;
        const totalWeightInput = row.querySelector('input[name$="[total_weight]"]');
        const totalWeight = parseFloat(totalWeightInput.value) || 0;
        
        let returnBalance = 0;
        
        // Calculate based on pure gold value - handle both string and number types
        if (pureGoldValue === '916') {
            returnBalance = totalWeight * 0.95;
        } else if (pureGoldValue === '835') {
            returnBalance = totalWeight * 0.87;
        } else if (pureGoldValue === '750W') {
            returnBalance = totalWeight * 0.78;
        } else if (pureGoldValue === '750R') {
            returnBalance = totalWeight * 0.78;
        } else if (pureGoldValue === '750Y') {
            returnBalance = totalWeight * 0.78;
        } else if (pureGoldValue === '375W') {
            returnBalance = totalWeight * 0.40;
        } else if (pureGoldValue === '375R') {
            returnBalance = totalWeight * 0.40;
        }
