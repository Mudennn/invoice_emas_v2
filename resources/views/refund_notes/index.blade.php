@extends('layouts.main', ['title' => 'Refund Notes'])

@section('content')
    <div class="table-header">
        <h1 class="h3 mb-0">Refund Notes</h1>
        <a href="{{ route('refund_notes.create') }}" class="primary-button">Create Refund Note</a>
    </div>
                ...
                <tbody>
                    @foreach ($refund_notes as $index => $refund_note)
                        <tr>
                            ...
                            <td>{{ \Carbon\Carbon::parse($refund_note->date)->format('d F Y') }}</td>
                            <td class="text-wrap"> {{ $refund_note->invoice->company_name ?? 'N/A' }}</td>
                            <td>
                            ...
                            <td class="text-wrap">{{ $refund_note->note }}</td>
                            <td class="text-center">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                        <li><a href="{{ route('refund_notes.edit', $refund_note->id) }}"
                                        ...
                                        <li><a href="{{ route('refund_notes.view', $refund_note->id) }}"
                                                class="dropdown-item"
                                                style="display: flex; justify-content: start; align-items: center; gap:8px; font-size:14px;"><span
                                                    class="material-symbols-outlined" style="font-size: 14px">
                                                    list_alt
