...
<div class="credit-note-form-container">
    <div class="left-form">
        <div class="input-form">
            <label class="form-label">Refund Note No</label>
            <input type="text" name="refund_note_no"
                value="{{ old('refund_note_no') ?? $refund_note->refund_note_no }}" class="form-control"
                {{ $ro }}>
            ...
            <select name="invoice_no" id="invoice_no" class="form-control form-select invoice-select2"
                ...
                @foreach ($invoices ?? [] as $invoice)
                    <option value="{{ $invoice->invoice_no }}"
                        {{ (old('invoice_no') ?? $refund_note->invoice_no) == $invoice->invoice_no ? 'selected' : '' }}>
                        {{ $invoice->invoice_no }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="input-form">
            ...
            <select name="reason" id="reason" class="form-control form-select" {{ $ro }}>
                ...
                @foreach ($reasons as $reason)
                    @if ($refund_note->reason)
                        <option value="{{ $reason->id }}"
                            {{ $reason->id == $refund_note->reason ? 'selected' : '' }}>
        ...
        <div class="input-form">
            ...
            <input type="date" name="date"
                value="{{ old('date', isset($refund_note->date) ? \Carbon\Carbon::parse($refund_note->date)->format('Y-m-d') : now()->timezone('Asia/Kuala_Lumpur')->format('Y-m-d')) }}"
...
<div class="mt-4">
    <div class="card-header d-flex justify-content-between align-items-center pb-3">
        <h5 class="mb-0">Refund Note Items</h5>
    </div>
    <div class="card-body">
        <div class="table-responsive" style="overflow-y: auto;" id="refund_notes-items">
            <table class="table table-hover table-bordered align-middle text-nowrap">
                ...
                <tbody>
                    @if (isset($refund_note->refundItems) && count($refund_note->refundItems) > 0)
                        @foreach ($refund_note->refundItems->where('status', '0') as $index => $item)
                            <tr class="refund-note-item" data-item-id="{{ $item->id }}"
                    ...
                    <tr>
                        ...
                        <th class="text-end">
                            <input type="number" step="0.01" name="subtotal"
                                ...
                                value="{{ old('subtotal') ?? ($refund_note->refundItems->first()->subtotal ?? '0.00') }}"
                                readonly>
                        </th>
                        <th><input type="number" step="0.01" name="total_return_balance"
                            class="form-control text-end total-return-balance-input"
                            value="{{ old('remark_total') ?? ($refund_note->refundItems->first()->remark_total ?? '0.00') }}" 
                            readonly></th>
                        @if (!isset($ro) || $ro === '')
                            <th></th>
                        @endif
...
<div class="input-form">
    ...
    <textarea name="note" class="form-control" {{ $ro }}>{{ old('note') ?? $refund_note->note }}</textarea>
