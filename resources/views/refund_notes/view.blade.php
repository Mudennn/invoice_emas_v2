@extends('layouts.main', ['title' => 'Refund Note Items for Refund Note #' . $refund_note->refund_note_no])

@section('content')
    <div class="table-header">
        <h1 class="h3 mb-0">Refund Note Items</h1>
        <a href="{{ route('refund_notes.index') }}" class="third-button">
    ...
    <div class="card">
        <div class="table-responsive" style="padding: 24px !important; min-height: 200px; overflow-y: auto;">
            <table class="table table-hover table-bordered align-middle text-nowrap"
                ...
                <tbody>
                    @forelse($refund_note->refundItems as $item)
                        <tr>
                            ...
                            <td class="text-wrap">{{ $item->kt ? $item->kt . ' - ' : '' }}
                            ...
                            <td class="text-end">
                                ...
                                @elseif($item->pure_gold == "375R")     
                                    <span> {{ number_format($item->total_weight, 2) }} × 0.40 =</span>
                                @endif
                                {{ $item->remark_total == 0 ? '' : $item->remark_total }}</td>
                            <td>
                ...
                <tfoot>
                    <tr>
                        ...
                        <th>
                            <div style="display: flex; justify-content: space-between;">
                                <span>RM</span>
                                <span>{{ number_format($refund_note->refundItems->first()->subtotal ?? 0, 2) }}</span>
                            </div>
                        </th>
                        <th>PG: {{ number_format($refund_note->refundItems->sum('remark_total'), 2) }}</th>
                    </tr>
                </tfoot>
            </table>
        </div>
