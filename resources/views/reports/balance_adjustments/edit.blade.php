@extends('layouts.main', ['title' => 'Edit Balance Adjustment'])

@section('content')
    <div class="table-header">
        <h1>Edit Gold Balance Adjustment</h1>
    </div>

    <div class="card">
        <div class="card-body">
            <form action="{{ route('reports.balance-adjustments.update', $adjustment->id) }}" method="POST">
                @csrf
                @method('PATCH')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="company_name" id="company_name" class="form-control @error('company_name') is-invalid @enderror" required>
                                ...
                                <option value="Habib Jewelry Manufacturing Sdn Bhd" {{ (old('company_name') ?? $adjustment->company_name) == 'Habib Jewelry Manufacturing Sdn Bhd' ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" name="effective_date" id="effective_date"
                                   ...
                                   value="{{ old('effective_date') ?? $adjustment->effective_date->format('Y-m-d') }}" required>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="adjustment_amount" id="adjustment_amount"
                                   ...
                                   value="{{ old('adjustment_amount') ?? $adjustment->adjustment_amount }}" step="0.01" min="0" required>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="adjustment_type" id="adjustment_type" class="form-control @error('adjustment_type') is-invalid @enderror" required>
                                ...
                                <option value="starting_balance" {{ (old('adjustment_type') ?? $adjustment->adjustment_type) == 'starting_balance' ? 'selected' : '' }}>Starting Balance</option>
                                <option value="stock_addition" {{ (old('adjustment_type') ?? $adjustment->adjustment_type) == 'stock_addition' ? 'selected' : '' }}>Stock Addition</option>
                                <option value="inventory_adjustment" {{ (old('adjustment_type') ?? $adjustment->adjustment_type) == 'inventory_adjustment' ? 'selected' : '' }}>Inventory Adjustment</option>
                ...
                <div class="form-group mt-3">
                    ...
                    <textarea name="description" id="description" rows="3"
                              ...
                              placeholder="Enter reason for this adjustment...">{{ old('description') ?? $adjustment->description }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // Set minimum date to allow historical dates
