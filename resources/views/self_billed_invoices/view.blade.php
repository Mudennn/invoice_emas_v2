@extends('layouts.main', ['title' => 'Self Billed Invoice Items' . ($selfBilledInvoiceNumber ? ' for Invoice #' . $selfBilledInvoiceNumber : '')])

@section('content')
    <div class="table-header">
        <h1>Self Billed Invoice Items</h1>
        <a href="{{ route('self_billed_invoices.index') }}" class="third-button">
        ...
        <div class="card-body">
            <div class="table-responsive" style="min-height: 200px; overflow-x: auto !important;">
                <table class="table table-hover table-bordered align-middle text-nowrap" style=" max-width: 100%; table-layout: fixed;">
                    ...
                    <tbody>
                        @foreach ($selfBilledInvoiceItems as $index => $item)
                            <tr>
                    ...
                    <tfoot>
                        @php
                            $subtotal = $selfBilledInvoiceItems->first()?->subtotal ?? 0;
                        @endphp
                        <tr>
                            ...
                            <th>PG: {{ number_format($selfBilledInvoiceItems->sum('remark_total'), 2) }}</th>
                        </tr>
                    </tfoot>
                </table>
            </div>
